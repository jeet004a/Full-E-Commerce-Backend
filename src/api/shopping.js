...
module.exports = (app) => {
    ...
    const customer = new CustomerService()
        //Create Order
    app.post('/shopping/order', UserAuth, AppLogs, async(req, res, next) => {
            try {
                const { _id } = req.user
        ...
        //Get Order Details
    app.get('/shopping/orders', UserAuth, AppLogs, async(req, res, next) => {
        try {
    ...
    //Get Cart Details
    app.get('/shopping/cart', UserAuth, AppLogs, async(req, res) => {
        try {
            const { data } = await customer.GetShopingDetails(req.user._id)
